...
Every package follows the same factory pattern — `createClient({ ... })` returns a typed client scoped to your app. Data-plane SDKs (Vault, Blaze) authenticate with an API key; `@nublestation/identity` is the exception — it rides the shared-cookie SSO session, so it takes no API key. See [its page](/docs/sdk/auth) for details.
