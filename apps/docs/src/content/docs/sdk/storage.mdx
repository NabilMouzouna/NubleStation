...
When your app wires in [Identity](/NubleStation/docs/services/auth), every file is **owned** by the user who uploaded it and is private by default. These methods give the Drive-style views and per-user sharing. They require a session cookie (sent automatically) — see [Vault — Access Control](/NubleStation/docs/services/storage-access) for the full model.
